    ...
    try {
       ...
       // if (await messageProcessor(this, m)) return;
        ...
        for (const name in global.plugins) {
            ...
            const isPluginFunction = typeof plugin === "function"
            const isPluginObject = typeof plugin.run === "function"
            
            if (!isPluginFunction && !isPluginObject) {
                continue
            }
            ...
            if ((usedPrefix = (match[0] || "")[0])) {
                ...
                const isAccept = cmdProperty instanceof RegExp ?
                    cmdProperty.test(command) :
                    Array.isArray(cmdProperty) ?
                    cmdProperty.some(cmd => cmd instanceof RegExp ?
                        cmd.test(command) : cmd === command) :
                    typeof cmdProperty === "string" ?
                    cmdProperty === command : false
                ...
                try {
                    ...
                    } else if (isPluginObject) {
                        await plugin.run.call(this, this, m, extra)
                    }
...
global.dfail = (type, m, conn) => {
    const msg = {
        ...
        unreg: `⚽ Para que puedas usar este comando, necesito que estés registrado/a, Regístrate usando\n> /reg nombre.edad `
    }[type]
